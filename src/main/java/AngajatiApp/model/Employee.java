...
public class Employee {
	...
	@Override
	public int hashCode() {
		return Objects.hash(cnp, firstName, lastName, function, salary);
	}
	...
	public static Employee getEmployeeFromString(String employeeString, int line) throws EmployeeException {
		...
		if (attributes.length != 6) {
			...
			if (attributes[DIDACTIC_FUNCTION_INDEX].equalsIgnoreCase("ASSISTENT")) {
				employee.setFunction(DidacticFunction.ASSISTENT);
			}
			if (attributes[DIDACTIC_FUNCTION_INDEX].equalsIgnoreCase("LECTURER")) {
				employee.setFunction(DidacticFunction.LECTURER);
			}
			if (attributes[DIDACTIC_FUNCTION_INDEX].equalsIgnoreCase("TEACHER")) {
				employee.setFunction(DidacticFunction.TEACHER);
			}
			if (attributes[DIDACTIC_FUNCTION_INDEX].equalsIgnoreCase("CONFERENTIAR")) {
